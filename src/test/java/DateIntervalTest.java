...
public class DateIntervalTest {
	...
	public void test_middlepointBounded() {
		assertEquals(parse("12.11.2005 22:20:15"), new DateInterval(
				...
				Interval.Bounded.CLOSED).getMidpoint()
		);
	...
	public void test_middlepointUnbounded() {
		...
		assertEquals(new Date(4611677853814364403L), new DateInterval(new Date(-16329226047000L), Interval.Unbounded.CLOSED_LEFT).getMidpoint());
		assertEquals(new Date(-4611694183040411404L), new DateInterval(new Date(-16329226047000L), Interval.Unbounded.CLOSED_RIGHT).getMidpoint());
		assertEquals(new Date(4611677853814364403L), new DateInterval(new Date(-16329226047000L), Interval.Unbounded.OPEN_LEFT).getMidpoint());
		assertEquals(new Date(-4611694183040411404L), new DateInterval(new Date(-16329226047000L), Interval.Unbounded.OPEN_RIGHT).getMidpoint());
	}

	@Test
	public void test_middlepointStartAndEndOffByOne() {
		long tmp = 128643893;
		assertEquals(new Date(tmp + 1), new DateInterval(new Date(tmp), new Date(tmp + 1), Interval.Bounded.CLOSED_RIGHT).getMidpoint());
		assertEquals(new Date(tmp), new DateInterval(new Date(tmp), new Date(tmp + 1), Interval.Bounded.CLOSED).getMidpoint());
		assertEquals(new Date(tmp), new DateInterval(new Date(tmp), new Date(tmp), Interval.Bounded.CLOSED).getMidpoint());
